...
from BADUC.core.config import PLAY_IMAGE_URL as START_IMAGE_URL
...
async def change_stream(chat_id):
    ...
    vid_url ="https://www.youtube.com/watch?v=DFVk71WxxBE"
...
💌 Rᴇǫᴜᴇsᴛᴇᴅ ʙʏ 💌  {requested_by}"""
    ...
    return await client.send_photo(chat_id, caption, reply_markup=buttons)
        ...
        if call_status == "PLAYING" or call_status == "PAUSED":
            try:
                position = await add_to_queue(
                    chat_id, user, title, duration, stream_file, stream_type
                )
...
🔉 Sᴛʀᴇᴀᴍ Tʏᴘᴇ 🔊  {stream_type}
💌 Rᴇǫᴜᴇsᴛᴇᴅ ʙʏ 💌  {requested_by}"""
                await app.send_photo(chat_id, caption)
                await stream_logger(
                    chat_id, user, title, duration, stream_type, position
                )
