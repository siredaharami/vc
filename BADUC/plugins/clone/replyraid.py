...
@Client.on_message(bad(["replyraid"]) & (filters.me | filters.user(SUDOERS)))
...
@Client.on_message(bad(["dreplyraid"]) & (filters.me | filters.user(SUDOERS)))
async def deactivate_reply_raid(c: Client, m: Message):
    ...
    if m.reply_to_message:
        ...
        Client = await m.reply_text("Reply Raid De-activating....")
        try:
            if u_id in que:
                que.remove(u_id)
                await Client.edit_text(f"Reply Raid has been De-activated on {username}")
                return
