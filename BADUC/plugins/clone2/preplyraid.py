...
async def activate_reply_raid(c: Client,m: Message):
    ...
    if m.reply_to_message_id:
        ...
        if u_id not in que:
            que.append(u_id)
            await Client.edit_text(f"Preply Raid has been activated on {username}")
        else:
...
async def deactivate_reply_raid(c: Client, m: Message):
    ...
    if m.reply_to_message:
        ...
        Client = await m.reply_text("Preply Raid De-activating....")
        try:
            if u_id in que:
                que.remove(u_id)
                await Client.edit_text(f"Preply Raid has been De-activated on {username}")
                return
