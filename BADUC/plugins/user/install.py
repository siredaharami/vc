...
os.makedirs("BADUC/plugins/user", exist_ok=True)


@app.on_message(bad(["install"]) & (filters.me | filters.user(SUDOERS)))
async def install_plugins(client: Client, message: Message):
    ...
    plugin_path = await message.reply_to_message.download("BADUC/plugins/user/")
...
@app.on_message(bad(["uninstall"]) & (filters.me | filters.user(SUDOERS)))
async def uninstall_plugins(client: Client, message: Message):
    ...
    plugin_path = f"BADUC/plugins/user/{plugin_name}.py"
