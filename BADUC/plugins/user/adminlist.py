...
@app.on_message(bad(["admins", "adminlist"]) & (filters.me | filters.user(SUDOERS)))
async def adminlist(app: Client, message: Message):
    replyid = None
    toolong = False
    ...
    admin = []
    badmin = []
    async for a in app.get_chat_members(
...
@app.on_message(bad(["reportadmin", "reportadmins", "report"]) & (filters.me | filters.user(SUDOERS)))
async def report_admin(app: Client, message: Message):
    await message.delete()
    if len(message.text.split()) >= 2:
        text = message.text.split(None, 1)[1]
    else:
        text = None
    grup = await app.get_chat(message.chat.id)
    admin = []
    async for a in app.get_chat_members(
        message.chat.id, filter=enums.ChatMembersFilter.ADMINISTRATORS
    ):
...
@app.on_message(bad(["bots", "botlist"]) & (filters.me | filters.user(SUDOERS)))
async def get_list_bots(app: Client, message: Message):
    replyid = None
    if len(message.text.split()) >= 2:
        chat = message.text.split(None, 1)[1]
        grup = await app.get_chat(chat)
    ...
    if message.reply_to_message:
        replyid = message.reply_to_message.id
    getbots = app.get_chat_members(chat)
    bots = []
    ...
    if replyid:
        await app.send_message(message.chat.id, teks, reply_to_message_id=replyid)
    else:
        await message.edit(teks)
