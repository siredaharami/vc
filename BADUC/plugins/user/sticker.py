...
@app.on_message(bad(["stickerid"]) & (filters.me | filters.user(SUDOERS)))
...
@app.on_message(bad(["getsticker"]) & (filters.me | filters.user(SUDOERS)))
...
@app.on_message(bad(["kang"]) & (filters.me | filters.user(SUDOERS)))
async def kang(client, message: Message):
    ...
    # single time...
    packnum = 0
    packname = f"f{message.from_user.id}_by_{message.from_user.id}"
    limit = 0
    try:
        while True:
